...
static void acs_dual_slot_auto_selects_picc(void) {
  const char readers[] =
      "ACS ACR1252 1S CL Reader SAM 00 00\0"
      "ACS ACR1252 1S CL Reader PICC 00 00\0"
      "\0";
  char selected[TEST_CAP_128];
  bool auto_selected = false;
  pcsc_fido_reader_pick_result_t result = pcsc_fido_pick_reader_from_list(
      readers, sizeof(readers), PCSC_FIDO_NULL, true, selected,
      sizeof(selected), &auto_selected);
  expect_true(result == PCSC_FIDO_READER_PICK_OK,
              "ACS dual-slot auto-select succeeds");
...
static void single_omnikey_selects_without_contactless_keyword(void) {
  const char readers[] =
      "HID Global OMNIKEY 5022 Smart Card Reader [OMNIKEY 5022 Smart Card "
      "Reader] 00 00\0"
      "\0";
  char selected[TEST_CAP_160];
  bool auto_selected = true;
  pcsc_fido_reader_pick_result_t result = pcsc_fido_pick_reader_from_list(
      readers, sizeof(readers), PCSC_FIDO_NULL, true, selected,
      sizeof(selected), &auto_selected);
  expect_true(result == PCSC_FIDO_READER_PICK_OK,
              "single OMNIKEY select succeeds");
  expect_true(!auto_selected,
              "single reader selection is not contactless auto-selection");
  expect_true(strcmp(selected,
                     "HID Global OMNIKEY 5022 Smart Card Reader [OMNIKEY 5022 "
                     "Smart Card Reader] "
                     "00 00") == 0,
              "OMNIKEY selected");
}
...
static void multiple_contactless_slots_stay_ambiguous(void) {
  const char readers[] =
      "Reader A PICC 00 00\0"
      "Reader B NFC 00 00\0"
      "\0";
  char selected[TEST_LIT_64];
  bool auto_selected = false;
  pcsc_fido_reader_pick_result_t result = pcsc_fido_pick_reader_from_list(
      readers, sizeof(readers), PCSC_FIDO_NULL, true, selected,
      sizeof(selected), &auto_selected);
...
static void acs_picc_and_generic_reader_stay_ambiguous(void) {
  ...
  expect_true(!auto_selected,
              "generic multi-reader selection is not model-specific");
}
...
static void explicit_needle_selects_matching_reader(void) {
  ...
  bool auto_selected = true;
  pcsc_fido_reader_pick_result_t result = pcsc_fido_pick_reader_from_list(
      readers, sizeof(readers), "Reader B", true, selected, sizeof(selected),
      &auto_selected);
  expect_true(result == PCSC_FIDO_READER_PICK_OK,
              "explicit reader needle selects one reader");
  expect_true(!auto_selected,
              "explicit selection is not contactless auto-selection");
  expect_true(strcmp(selected, "Reader B NFC 00 00") == 0,
              "explicit reader selected");
}
...
static void explicit_needle_is_case_insensitive(void) {
  ...
  bool auto_selected = true;
  pcsc_fido_reader_pick_result_t result = pcsc_fido_pick_reader_from_list(
      readers, sizeof(readers), "picc", true, selected, sizeof(selected),
      &auto_selected);
  expect_true(result == PCSC_FIDO_READER_PICK_OK,
              "lowercase explicit reader needle selects PICC");
  expect_true(!auto_selected,
              "explicit case-insensitive selection is not auto-selection");
  expect_true(strcmp(selected, "ACS ACR1252 1S CL Reader PICC 00 00") == 0,
...
static void no_match_cases(void) {
  ...
  bool auto_selected = false;
  expect_true(
      pcsc_fido_pick_reader_from_list(
          empty, sizeof(empty), PCSC_FIDO_NULL, true, selected,
          sizeof(selected), &auto_selected) == PCSC_FIDO_READER_PICK_NO_MATCH,
      "empty reader list is NO_MATCH");
  expect_true(
      pcsc_fido_pick_reader_from_list(
          readers, sizeof(readers), "missing", true, selected, sizeof(selected),
...
static void auto_select_disabled(void) {
  ...
  bool auto_selected = false;
  pcsc_fido_reader_pick_result_t result = pcsc_fido_pick_reader_from_list(
      readers, sizeof(readers), PCSC_FIDO_NULL, false, selected,
      sizeof(selected), &auto_selected);
  expect_true(result == PCSC_FIDO_READER_PICK_AMBIGUOUS,
              "auto-select disabled stays ambiguous");
}

static void nfc_and_contactless_keywords(void) {
  const char nfc_readers[] =
      "Vendor NFC Reader 00 00\0"
      "Vendor SAM Slot SAM 01 00\0"
      "\0";
  const char contactless_readers[] =
      "Vendor Contactless Reader 00 00\0"
      "Vendor SAM Slot SAM 01 00\0"
      "\0";
  char selected[TEST_CAP_128];
  bool auto_selected = false;
  expect_true(
      pcsc_fido_pick_reader_from_list(
          nfc_readers, sizeof(nfc_readers), PCSC_FIDO_NULL, true, selected,
          sizeof(selected), &auto_selected) == PCSC_FIDO_READER_PICK_OK,
      "single NFC slot auto-selects");
  expect_true(auto_selected, "NFC auto-selection flagged");
  auto_selected = false;
  expect_true(pcsc_fido_pick_reader_from_list(
                  contactless_readers, sizeof(contactless_readers),
                  PCSC_FIDO_NULL, true, selected, sizeof(selected),
...
static void sam_only_reader_no_match(void) {
  ...
  expect_true(
      pcsc_fido_pick_reader_from_list(
          readers, sizeof(readers), PCSC_FIDO_NULL, true, selected,
          sizeof(selected), &auto_selected) == PCSC_FIDO_READER_PICK_AMBIGUOUS,
...
static void truncates_output_buffer(void) {
  ...
  char selected[TEST_LIT_8];
  bool auto_selected = false;
  expect_true(pcsc_fido_pick_reader_from_list(
                  readers, sizeof(readers), PCSC_FIDO_NULL, true, selected,
                  sizeof(selected),
                  &auto_selected) == PCSC_FIDO_READER_PICK_NAME_TOO_LONG,
...
static void empty_needle_matches_all(void) {
  ...
  char selected[TEST_LIT_64];
  bool auto_selected = false;
  expect_true(pcsc_fido_pick_reader_from_list(readers, sizeof(readers), "",
                                              true, selected, sizeof(selected),
                                              &auto_selected) ==
...
static void reader_status_filters(void) {
  ...
  expect_true(
      pcsc_fido_reader_status_has_card((uint32_t)TEST_SCARD_STATE_PRESENT),
      "present state has card");
  expect_true(!pcsc_fido_reader_status_has_card(
                  (uint32_t)(TEST_SCARD_STATE_PRESENT | TEST_SCARD_STATE_MUTE)),
              "mute card rejected");
  expect_true(!pcsc_fido_reader_status_has_card((
...
static void length_aware_reader_helpers(void) {
  ...
  expect_true(pcsc_fido_reader_name_contains_ci_len(reader, sizeof(reader),
                                                    needle, sizeof(needle)),
              "length-aware contains handles unterminated buffers");
  expect_true(
      pcsc_fido_reader_name_is_contactless_slot_len(reader, sizeof(reader)),
...
static void reader_list_helpers_cover_edge_cases(void) {
  ...
  expect_true(pcsc_fido_reader_list_is_valid(readers, sizeof(readers)),
              "valid MSZ accepted");
  expect_true(!pcsc_fido_reader_list_is_valid(PCSC_FIDO_NULL, sizeof(readers)),
              "null list invalid");
  expect_true(!pcsc_fido_reader_list_is_valid(readers, 0u),
              "zero length invalid");

  result = pcsc_fido_reader_list_next(readers, sizeof(readers), &off, &entry,
                                      &entry_len);
  ...
  result = pcsc_fido_reader_list_next(readers, sizeof(readers), &off, &entry,
                                      &entry_len);
  expect_true(result == PCSC_FIDO_READER_LIST_ENTRY_END,
              "MSZ terminator ends iteration");
  expect_true(pcsc_fido_reader_list_next(PCSC_FIDO_NULL, sizeof(readers), &off,
                                         &entry, &entry_len) ==
...
static void reader_env_needle_prefers_explicit_value(void) {
  ...
  expect_true(
      pcsc_fido_reader_env_needle(PCSC_FIDO_NULL) != PCSC_FIDO_NULL &&
          strcmp(pcsc_fido_reader_env_needle(PCSC_FIDO_NULL), "from-env") == 0,
...
static void pick_respects_readers_len_not_buffer_size(void) {
  ...
  char selected[TEST_LIT_64];
  bool auto_selected = false;
  const size_t pcsc_list_len =
      sizeof("Reader A PICC 00 00"); /* name + first NUL only */

  pcsc_fido_fill_bytes(readers, sizeof(readers), (uint8_t)'X');
  (void)pcsc_fido_copy_bytes(readers, sizeof(readers), 0u,
                             "Reader A PICC 00 00", pcsc_list_len);
  (void)pcsc_fido_copy_bytes(
      readers + pcsc_list_len, sizeof("Reader B NFC 00 00"), 0u,
      "Reader B NFC 00 00", sizeof("Reader B NFC 00 00"));
  readers[pcsc_list_len + sizeof("Reader B NFC 00 00")] = '\0';

  expect_true(
      pcsc_fido_pick_reader_from_list(
          readers, pcsc_list_len, PCSC_FIDO_NULL, true, selected,
          sizeof(selected), &auto_selected) == PCSC_FIDO_READER_PICK_NO_MATCH,
...
static void malformed_reader_list_is_bounded(void) {
  ...
  char selected[TEST_LIT_64];
  bool auto_selected = true;
  pcsc_fido_fill_bytes(readers, sizeof(readers), (uint8_t)'A');
  expect_true(
      pcsc_fido_pick_reader_from_list(
          readers, sizeof(readers), PCSC_FIDO_NULL, true, selected,
          sizeof(selected), &auto_selected) == PCSC_FIDO_READER_PICK_NO_MATCH,
  ...
  pcsc_fido_fill_bytes(readers, sizeof(readers), (uint8_t)'B');
  (void)pcsc_fido_copy_bytes(readers, sizeof(readers), 0u, "Reader PICC 00 00",
                             sizeof("Reader PICC 00 00"));
...
int main(void) {
  expect_true(!pcsc_fido_reader_name_contains_ci(PCSC_FIDO_NULL, "picc"),
              "PCSC_FIDO_NULL reader rejected");
  expect_true(!pcsc_fido_reader_name_contains_ci("ACS PICC", PCSC_FIDO_NULL),
              "PCSC_FIDO_NULL needle rejected");
  expect_true(pcsc_fido_reader_name_contains_ci("ACS PICC", ""),
              "empty needle matches");
  expect_true(!pcsc_fido_reader_name_is_contactless_slot(PCSC_FIDO_NULL),
              "PCSC_FIDO_NULL contactless check");
  expect_true(pcsc_fido_reader_name_contains_ci("ACS PICC", "picc"),
              "case-insensitive contains");
  expect_true(
      pcsc_fido_reader_name_is_contactless_slot("ACS ACR1252 PICC 00 00"),
      "PICC is contactless");
  expect_true(!pcsc_fido_reader_name_is_contactless_slot(
                  "Generic PC/SC Smart Card Reader 02 00"),
              "generic reader is not hard-coded as contactless");
  expect_true(
      !pcsc_fido_reader_name_is_contactless_slot("ACS ACR1252 SAM 00 00"),
      "SAM is not contactless");
  expect_true(
      pcsc_fido_reader_name_is_sam_slot_len(
          "ACS ACR1252 SAM 00 00", sizeof("ACS ACR1252 SAM 00 00") - 1u),
