...
static void config_defaults_to_tap_arm(void) {
  ...
  expect_true(cfg.virtual_key_mode == PCSC_FIDO_VIRTUAL_KEY_TAP_ARM,
              "default mode tap-arm");
...
static void config_invalid_numeric_env_falls_back(void) {
  pcsc_fido_browser_config_t cfg;
  unsetenv("PCSC_FIDO_VIRTUAL_KEY");
  expect_true(setenv("PCSC_FIDO_ARM_SEC", "not-a-number", 1) == 0,
              "set invalid arm sec");
  expect_true(pcsc_fido_load_browser_config(&cfg),
              "invalid numeric env falls back");
  expect_true(cfg.arm_sec == PCSC_FIDO_ARM_SEC_DEFAULT,
              "invalid arm sec uses default");
}

static void config_mode_name_covers_all_values(void) {
  expect_true(
      strcmp(pcsc_fido_virtual_key_mode_name(PCSC_FIDO_VIRTUAL_KEY_TAP_ARM),
             "tap-arm") == 0,
      "tap-arm mode name");
  expect_true(
      strcmp(pcsc_fido_virtual_key_mode_name(PCSC_FIDO_VIRTUAL_KEY_ALWAYS),
             "always") == 0,
      "always mode name");
  {
    const int unknown_mode = TEST_LIT_99;
    pcsc_fido_virtual_key_mode_t mode;
    pcsc_fido_zero_bytes(&mode, sizeof(mode));
    expect_true(pcsc_fido_copy_bytes(&mode, sizeof(mode), 0u, &unknown_mode,
                                     sizeof(unknown_mode) < sizeof(mode)
                                         ? sizeof(unknown_mode)
                                         : sizeof(mode)),
                "load out-of-range mode bits");
    expect_true(strcmp(pcsc_fido_virtual_key_mode_name(mode), "unknown") == 0,
                "unknown mode name");
  }
}

static void config_print_rejects_null(void) {
  pcsc_fido_print_browser_config(PCSC_FIDO_NULL, PCSC_FIDO_NULL);
...
static void config_arm_sec_out_of_range_falls_back(void) {
  ...
  expect_true(pcsc_fido_load_browser_config(&cfg),
              "oob arm sec loads with fallback");
  expect_true(cfg.arm_sec == PCSC_FIDO_ARM_SEC_DEFAULT,
              "oob arm sec uses default");
}
...
static void config_parses_always_mode(void) {
  ...
  expect_true(cfg.virtual_key_mode == PCSC_FIDO_VIRTUAL_KEY_ALWAYS,
              "always mode");
  expect_true(cfg.arm_sec == TEST_LIT_45U, "arm sec parsed");
}

static void config_rejects_invalid_mode(void) {
  pcsc_fido_browser_config_t cfg;
  expect_true(setenv("PCSC_FIDO_VIRTUAL_KEY", "sometimes", 1) == 0,
              "set invalid mode");
...
static void config_print_includes_mode(void) {
  ...
  if (out != PCSC_FIDO_NULL) {
    ...
    expect_true(fgets(line, sizeof(line), out) != PCSC_FIDO_NULL && errno == 0,
                "first config line");
    expect_true(strstr(line, "PCSC_FIDO_VIRTUAL_KEY=tap-arm") != PCSC_FIDO_NULL,
                "printed mode");
    (void)fclose(out);
  } else {
