...
static bool commit_session(SCARDCONTEXT ctx, SCARDHANDLE card,
                           const SCARD_IO_REQUEST* send_pci,
                           const char* reader) {
  bool ok = true;
  ...
  if (reader != PCSC_FIDO_NULL) {
    ok =
        pcsc_fido_copy_cstr(g_session.reader, sizeof(g_session.reader), reader);
    if (!ok) {
...
bool pcsc_fido_session_is_ready(void) {
  bool ready;
  pthread_mutex_lock(&g_session.lock);
  ready = g_session.card != 0 && g_session.send_pci != PCSC_FIDO_NULL;
  pthread_mutex_unlock(&g_session.lock);
  return ready;
}
...
bool pcsc_fido_session_verify_ready(char* err, size_t err_cap) {
  ...
  LONG rv;
  pthread_mutex_lock(&g_session.lock);
  if (g_session.card == 0 || g_session.send_pci == PCSC_FIDO_NULL) {
    pthread_mutex_unlock(&g_session.lock);
    return false;
  }
  card = g_session.card;
  pthread_mutex_unlock(&g_session.lock);
  rv = SCardStatus(card, PCSC_FIDO_NULL, PCSC_FIDO_NULL, &state, &protocol,
...
bool pcsc_fido_session_snapshot_tx(pcsc_fido_session_tx_t* tx, char* err,
                                   size_t err_cap) {
  bool ok;
  if (tx == PCSC_FIDO_NULL) {
    pcsc_fido_set_err(err, err_cap, "invalid PC/SC session snapshot arguments");
    return false;
  }
  pthread_mutex_lock(&g_session.lock);
  ok = g_session.card != 0 && g_session.send_pci != PCSC_FIDO_NULL;
  if (ok) {
...
bool pcsc_fido_session_tx_is_current(const pcsc_fido_session_tx_t* tx) {
  bool ok;
  if (tx == PCSC_FIDO_NULL || tx->card == 0) {
    return false;
  }
  pthread_mutex_lock(&g_session.lock);
...
void pcsc_fido_session_reset(void) {
  ...
  g_session.ctx = 0;
  g_session.card = 0;
  g_session.send_pci = PCSC_FIDO_NULL;
...
static bool transmit_apdu(SCARDHANDLE card, const SCARD_IO_REQUEST* send_pci,
                          const uint8_t* capdu, size_t capdu_len,
                          uint8_t* rapdu, size_t rapdu_cap, size_t* rapdu_len,
                          char* err, size_t err_cap) {
  DWORD recv_len = (DWORD)rapdu_cap;
  LONG rv;
  if (send_pci == PCSC_FIDO_NULL || capdu == PCSC_FIDO_NULL ||
  ...
  rv = SCardTransmit(card, send_pci, capdu, (DWORD)capdu_len, PCSC_FIDO_NULL,
                     rapdu, &recv_len);
  if (rv != SCARD_S_SUCCESS) {
...
static bool session_transmit_apdu(void* ctx, const uint8_t* capdu,
                                  size_t capdu_len, uint8_t* rapdu,
                                  size_t rapdu_cap, size_t* rapdu_len,
                                  char* err, size_t err_cap) {
  ...
  return transmit_apdu(tx->card, tx->send_pci, capdu, capdu_len, rapdu,
...
bool pcsc_fido_session_transmit_chained(const pcsc_fido_session_tx_t* tx,
                                        const uint8_t* capdu, size_t capdu_len,
  ...
  bool current;
  pthread_mutex_lock(&g_session.lock);
  if (tx == PCSC_FIDO_NULL || g_session.card != tx->card ||
      g_session.send_pci != tx->send_pci ||
      g_session.generation != tx->generation) {
    pthread_mutex_unlock(&g_session.lock);
    pcsc_fido_set_err(err, err_cap,
                      "PC/SC session no longer matches transmit handles");
    return false;
  }
  ...
  pthread_mutex_unlock(&g_session.lock);
  ok = pcsc_fido_apdu_transmit_chained_cancel(
      &ctx, session_transmit_apdu, session_cancel_check, PCSC_FIDO_NULL, capdu,
      capdu_len, rapdu, rapdu_cap, rapdu_len, err, err_cap);
  pthread_mutex_lock(&g_session.lock);
...
static bool connect_card(SCARDCONTEXT ctx, const char* reader,
                         SCARDHANDLE* card, const SCARD_IO_REQUEST** send_pci,
                         char* err, size_t err_cap) {
  DWORD active = 0u;
  if (reader == PCSC_FIDO_NULL || card == PCSC_FIDO_NULL ||
      send_pci == PCSC_FIDO_NULL) {
  ...
  LONG rv = SCardConnect(ctx, reader, SCARD_SHARE_SHARED,
                         SCARD_PROTOCOL_T0 | SCARD_PROTOCOL_T1, card, &active);
  if (rv == SCARD_E_PROTO_MISMATCH) {
    rv = SCardConnect(ctx, reader, SCARD_SHARE_SHARED, SCARD_PROTOCOL_RAW, card,
                      &active);
  }
  if (rv != SCARD_S_SUCCESS) {
...
static bool select_fido(SCARDHANDLE card, const SCARD_IO_REQUEST* send_pci,
                        char* err, size_t err_cap) {
  {
    uint8_t apdu[PCSC_FIDO_SELECT_PROBE_APDU_CAP];
    ...
    if (pcsc_fido_pack_select_fido_apdu(apdu, sizeof(apdu), &apdu_len, true) &&
        pcsc_fido_apdu_transmit_chained(&tx_ctx, session_transmit_apdu, apdu,
                                        apdu_len, rapdu, sizeof(rapdu),
                                        &rapdu_len, err, err_cap) &&
        pcsc_fido_apdu_status_word(rapdu, rapdu_len) == PCSC_FIDO_SW_OK) {
      pcsc_fido_log(PCSC_FIDO_LOG_DEBUG,
                    "FIDO SELECT preferred returned SW=9000");
      return true;
    }
    if (pcsc_fido_pack_select_fido_apdu(apdu, sizeof(apdu), &apdu_len, false) &&
        ...
        pcsc_fido_apdu_status_word(rapdu, rapdu_len) == PCSC_FIDO_SW_OK) {
      pcsc_fido_log(PCSC_FIDO_LOG_DEBUG,
                    "FIDO SELECT fallback returned SW=9000");
      return true;
    }
  }
...
bool pcsc_fido_session_ensure(const char* reader_needle, char* err,
                              size_t err_cap) {
  SCARDCONTEXT ctx = 0;
  SCARDHANDLE card = 0;
  const SCARD_IO_REQUEST* send_pci = PCSC_FIDO_NULL;
  char reader[PCSC_FIDO_READER_NAME_MAX];
  ...
  pthread_mutex_unlock(&g_session.lock);
  if (!pcsc_fido_reader_select_and_wait(ctx, reader_needle, reader,
                                        sizeof(reader), err, err_cap) ||
