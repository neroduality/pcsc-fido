...
const uint8_t PCSC_FIDO_EMPTY_CLIENT_DATA_HASH[PCSC_FIDO_CLIENT_DATA_HASH_LEN] =
...
bool pcsc_fido_daemon_is_get_assertion(uint8_t hid_cmd, const uint8_t* payload,
                                       size_t payload_len) {
  return hid_cmd == PCSC_FIDO_HID_CMD_CBOR && payload != PCSC_FIDO_NULL &&
         payload_len > 0u && payload[0] == PCSC_FIDO_CTAP2_CMD_GET_ASSERTION;
}

bool pcsc_fido_daemon_get_assertion_has_empty_client_data_hash(
    const uint8_t* payload, size_t payload_len) {
  size_t off = 1u;
  size_t pairs = 0u;
  if (payload == PCSC_FIDO_NULL || payload_len == 0u ||
      payload[0] != PCSC_FIDO_CTAP2_CMD_GET_ASSERTION) {
    return false;
  }
  ...
  // bytes inside a credential ID or other field can never trigger a false
  // match.
  ...
  for (size_t i = 0u; i < pairs; i++) {
    ...
    if (key == PCSC_FIDO_GET_ASSERTION_KEY_CLIENT_DATA_HASH) {
      ...
      return hash_len == PCSC_FIDO_CLIENT_DATA_HASH_LEN &&
             pcsc_fido_span_ok(off, hash_len, payload_len) &&
             memcmp(payload + off, PCSC_FIDO_EMPTY_CLIENT_DATA_HASH,
                    hash_len) == 0;
    }
...
bool pcsc_fido_daemon_is_terminal_webauthn_request(uint8_t cmd,
                                                   const uint8_t* payload,
                                                   size_t payload_len) {
  if (cmd != PCSC_FIDO_HID_CMD_CBOR || payload == PCSC_FIDO_NULL ||
      payload_len == 0u) {
    return false;
  }
  if (payload[0] == PCSC_FIDO_CTAP2_CMD_MAKE_CREDENTIAL) {
    return true;
  }
  return payload[0] == PCSC_FIDO_CTAP2_CMD_GET_ASSERTION &&
