...
static bool write_uhid_event(int fd, const struct uhid_event* ev) {
  ...
  while (remaining > 0u) {
    ...
    if (wrote < 0) {
      ...
      pcsc_fido_log(PCSC_FIDO_LOG_ERROR, "UHID write failed: %s",
                    strerror(errno));
      return false;
    }
    if (wrote == 0) {
...
static bool send_packet(int fd,
  ...
  if (!pcsc_fido_copy_bytes(ev.u.input2.data, sizeof(ev.u.input2.data), 0u,
                            packet, PCSC_FIDO_HID_PACKET_SIZE)) {
    return false;
  }
  return write_uhid_event(fd, &ev);
}

bool pcsc_fido_daemon_send_hid_response(int fd, uint32_t cid, uint8_t cmd,
  ...
  copied = payload_len < PCSC_FIDO_HID_INIT_PAYLOAD_MAX
               ? payload_len
  ...
  while (copied < payload_len) {
    ...
    if (!pcsc_fido_hid_encode_cont_packet(cid, seq, payload + copied, chunk,
                                          packet) ||
...
bool pcsc_fido_daemon_send_hid_error(int fd, uint32_t cid, uint8_t code) {
  return pcsc_fido_daemon_send_hid_response(fd, cid, PCSC_FIDO_HID_CMD_ERROR,
                                            &code, 1u);
}

bool pcsc_fido_daemon_send_keepalive(int fd, uint32_t cid) {
  const uint8_t processing = PCSC_FIDO_UHID_KEEPALIVE_STATUS_PROCESSING;
  return pcsc_fido_daemon_send_hid_response(
      fd, cid, PCSC_FIDO_HID_CMD_KEEPALIVE, &processing, 1u);
}
...
bool pcsc_fido_daemon_create_uhid_device(int fd) {
  ...
  (void)pcsc_fido_copy_cstr((char*)ev.u.create2.phys, sizeof(ev.u.create2.phys),
                            "pcsc-fido/uhid");
  ev.u.create2.rd_size = sizeof(K_FIDO_REPORT_DESC);
  if (!pcsc_fido_copy_bytes(ev.u.create2.rd_data, sizeof(ev.u.create2.rd_data),
                            0u, K_FIDO_REPORT_DESC,
                            sizeof(K_FIDO_REPORT_DESC))) {
    return false;
  }
  ev.u.create2.bus = BUS_USB;
  ev.u.create2.vendor = PCSC_FIDO_UHID_USB_VENDOR_ID;
  ev.u.create2.product = PCSC_FIDO_UHID_USB_PRODUCT_ID;
...
bool pcsc_fido_daemon_output_packet_data(const struct uhid_event* ev,
                                         const uint8_t** data) {
  const uint8_t* packet_data;
  size_t size;
  if (ev == PCSC_FIDO_NULL || data == PCSC_FIDO_NULL ||
      ev->type != UHID_OUTPUT) {
    return false;
  }
