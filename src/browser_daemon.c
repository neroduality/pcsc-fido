...
static void print_usage(FILE* out) {
  ...
  pcsc_fido_io_printf(
      out,
      "Usage: pcsc-fido [--list-readers|--print-config|--version|--help]\n"
      "\n"
      "Without arguments, start the virtual HID FIDO bridge daemon.\n"
      "  --list-readers   list PC/SC readers visible to pcscd\n"
      "  --print-config   print resolved daemon configuration from the "
      "environment\n"
...
static int handle_cli(int argc, char** argv) {
  ...
  if (argc <= 0 || arg_count <= 1) {
    return -1;
  }
  if (argv == PCSC_FIDO_NULL) {
    print_usage(stderr);
    return EXIT_USAGE_ERROR;
  }
  pcsc_fido_zero_bytes(err, sizeof(err));
  if (argc == ARGC_WITH_ONE_OPTION && strcmp(argv[1], "--help") == 0) {
    print_usage(stdout);
    return 0;
  }
  if (argc == ARGC_WITH_ONE_OPTION && strcmp(argv[1], "--version") == 0) {
    pcsc_fido_io_printf(stdout, "pcsc-fido %s\n", PCSC_FIDO_VERSION);
    return 0;
  }
  if (argc == ARGC_WITH_ONE_OPTION && strcmp(argv[1], "--print-config") == 0) {
  ...
  if (argc == ARGC_WITH_ONE_OPTION && strcmp(argv[1], "--list-readers") == 0) {
    if (!pcsc_fido_bridge_list_readers(stdout, err, sizeof(err))) {
      pcsc_fido_log(PCSC_FIDO_LOG_ERROR, "%s",
                    err[0] != '\0' ? err : "failed to list readers");
      return 1;
    }
    return 0;
  }
...
static int run_daemon(int uhid_fd) {
  ...
  if (!pcsc_fido_daemon_signals_init()) {
    pcsc_fido_log(PCSC_FIDO_LOG_ERROR, "%s: %s", "pcsc-fido signal setup",
                  strerror(errno));
    return 1;
  }
  if (!pcsc_fido_load_browser_config(&cfg)) {
...
int pcsc_fido_browser_daemon_main(int argc, char** argv) {
  ...
  if (fd < 0) {
    pcsc_fido_log(PCSC_FIDO_LOG_ERROR, "%s: %s", "open /dev/uhid",
                  strerror(errno));
    return 1;
  }
#if defined(PCSC_FIDO_TESTING)
