...
| `daemon_signals.c` | `sigaction` handlers set an atomic stop flag and write a wake byte to a self-pipe; poll loops watch the pipe's read end for immediate shutdown wake (bounded poll timeouts remain as a backstop). |
...
| `pcsc_log.c` | Leveled `pcsc_fido_log()` via `pcsc_fido_io_*` (`DEBUG` gated by `PCSC_FIDO_DEBUG`). |
| `pcsc_fido_io.c` | Sole stream I/O sink (`fprintf`/`fputs`/`fputc` wrappers). |
...
1. **Read + reassemble.** `daemon_uhid_loop.c` polls `/dev/uhid`, dispatches
   ...
   `PCSC_FIDO_CTAPHID_MAX_FRAMED_PAYLOAD` (7609 = 57 + 128x59, the framing
...
3. **Single-flight worker.** `exchange_orchestrator.c` runs one
   ...
   KEEPALIVE (100 ms, per CTAP 2.3 §11.2.9.1.7), drains CANCEL/close into
   `SCardCancel()`, and answers
...
**Host hardening:** All builds apply the [OpenSSF Compiler Options Hardening Guide](https://best.openssf.org/Compiler-Hardening-Guides/Compiler-Options-Hardening-Guide-for-C-and-C++.html) compile/link set via the org lint kit's generated `cmake/Hardening.cmake` + `cmake/CompilerHardeningProbes.cmake` (do not hand-edit; regenerated fail-on-change by `make lint`). `define_hardening` creates an INTERFACE target that `pcsc_fido_core` links `PUBLIC`, so the daemon and every test/fuzz target inherit the flags: blanket `-Werror` with `-Wall -Wextra -Wconversion -Wsign-conversion -Wformat=2`, `-O2`, PIE, `_FORTIFY_SOURCE=3` on Release-family builds (auto-suppressed under sanitizers or `-fhardened`), stack protector/clash protection, CET or AArch64 branch protection (native, probe-gated), `-fstrict-flex-arrays=3`, `-ftrivial-auto-var-init=zero`, `-fno-strict-aliasing`/`-fno-strict-overflow`/`-fno-delete-null-pointer-checks`, and RELRO/NOW/noexecstack/nodlopen/`--as-needed` linker hygiene. Enforcement is verified, not assumed: the kit audits the actual per-TU command lines in `compile_commands.json` and the link lines (`build.ninja`/`link.txt`) on every lint run. Release LTO when sanitizers and coverage are off.
